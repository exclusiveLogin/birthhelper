...
export class AdSliderComponent implements OnInit, OnDestroy {
    ...
    @Input() set settings(value: SliderSettings) {
        ...
        if ( value.backgroundColor ) {
            this.wrapperStyle.backgroundColor = value.backgroundColor + value.backgroundColorOpacity ?? 'ff';
        }
    ...
    ngOnInit(): void {
        const obs = new ResizeObserver((entries) => {
            ...
            if (!ent) { return; }
            this.showPicture$.next(ent.contentRect.width > 640 && !!this.pictureStyle?.backgroundImage);
        });
