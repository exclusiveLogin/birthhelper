    ...
    getImage(image: IImage | MetaPhoto): Observable<SafeUrl> {
        ...
        return of(mainURL).pipe(
            switchMap((url) =>
                this.checkCache(url)
                    ? this.getCache(url)
                    : this.saveCache(url, this.rest.getRaw(url))
            ),
            catchError((err) =>
                this.checkCache(fallbackURL)
                    ? this.getCache(fallbackURL)
                    : this.saveCache(fallbackURL, this.rest.getRaw(fallbackURL))
            ),
            catchError((err) =>
                this.checkCache(staticBlank)
                    ? this.getCache(staticBlank)
                    : this.saveCache(staticBlank, this.rest.getRaw(staticBlank))
            ),
            map((data: ArrayBuffer) =>
                URL.createObjectURL(new Blob([data], { type: "image/jpg" }))
            ),
            map((url: string) => this.sanitizer.bypassSecurityTrustUrl(url)),
            tap((url: SafeUrl) =>
                Promise.resolve(() => URL.revokeObjectURL(url as string))
            )
        );
    }
    getImage$(
        image: IImage | MetaPhoto
    ): [Observable<SafeUrl>, BehaviorSubject<null>] {
        let cursor = -1;
