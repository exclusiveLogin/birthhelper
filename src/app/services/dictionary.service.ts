...
export class DictionaryService {
    ...
    getDictStream(name, filters: Record<string, string>, page = 1): Observable<IDictItem[]> {
        if (this.dictStreams$[name]) { return this.dictStreams$[name]; }

        const dict$ = this.rest.fetchDictionary(name, filters, page)
            .pipe(
