...
export class CartComponent implements OnInit {
    ...
    tpl_suggestion: TemplateRef<any>;
    ...
    hasPendingOrders$ = this.validationTree$.pipe(
        map((tree) =>
            tree.reduce((acc, cur) => [...acc, ...cur._orders], [] as Order[])
        ),
    ...
    totalPrice$: Observable<number> = this.validationTree$.pipe(
        map((tree) =>
            tree.reduce((keys, cur) => [...keys, ...cur._orders], [] as Order[])
        ),
    ...
    totalPriceValid$: Observable<number> = merge(
        this.orderService.onSlots$,
        of(null)
    ).pipe(
    ...
    attemptSubmitCart(): void {
        this.authService.onUserAccess$
            .pipe(take(1))
            .subscribe((userMode) =>
                userMode
                    ? this.openDialogContacts()
                    : this.openDialogSuggestion()
            );
    }

    openDialogSuggestion(): void {
        this.dialogService.showDialogByTemplate(this.tpl_suggestion, {
            mode: "dialog",
        });
    }
    ...
    openDialogContacts(): void {
        this.authService.user$.pipe(take(1)).subscribe((user) => {
            ...
            this.dialogService.showDialogByTemplate(this.tpl_contacts, {
                mode: "dialog",
            });
        });
    }
