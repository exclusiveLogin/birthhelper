...
export class ContragentComponent implements OnInit {
    ...
    @Input() set validationTreeByContragent(value: ValidationTreeContragent) {
        ...
        this.sections = value.sections.map((sc) => ({
            key: sc,
            title: Sections[sc],
        }));
        value.sections.forEach(
            (s, idx) =>
                (this.sectionTabs[s] = value.sectionConfigs[idx].tabs.map(
                    (t) => ({ key: t.key, title: t.title })
                ))
        );
    }
    @Input() show = false;
    ...
    getOrders(section: SectionType, tabKey: string): Order[] {
        return this._validationTreeByContragent._orders.filter(
            (o) => o.section_key === section && o.tab_key === tabKey
        );
    }
    async gotoConfigurator() {
        const contragentData =
            this.ordersService.contragentHashMap[
                this._validationTreeByContragent.contragentHash
            ];
        if (!this._validationTreeByContragent) {
            return;
        }
        await this.router.navigate([
            "/system",
            "configurator",
            this._validationTreeByContragent.sections[0],
            contragentData.id,
        ]);
    }
    ...
    ngOnInit(): void {
        ...
        this.contragent$ = contragentData
            ? this.restService
                  .getEntity<IClinicSrc>("ent_contragents", contragentData.id)
