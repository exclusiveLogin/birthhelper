...
export class SearchComponent implements OnInit, AfterViewInit {
    ...
    onHashByFilters$ = combineLatest([
        this.onFilters$,
        this.onInitSectionType$,
    ]).pipe(
        switchMap(([filters, sectionKey]) =>
            filters
                ? this.provider.getFilterHash(sectionKey, filters)
                : of(null)
        ),
        tap((h) => {
    ...
    dataProvider$ = this.onInitSectionType$.pipe(
        map((section) => this.provider.getListProvider(section))
    );
    setProvider$ = this.onInitSectionType$.pipe(
        map((section) => this.provider.getSetProvider(section))
    );
    filterProvider$ = this.onInitSectionType$.pipe(
        map((section) => this.provider.getFilterProvider(section))
    );
    filterConfigProvider$ = this.onInitSectionType$.pipe(
        map((section) => this.provider.getFilterConfigProvider(section))
    );

    mainSet$ = this.onHash$.pipe(
        switchMap((hash) =>
            this.setProvider$.pipe(switchMap((provider) => provider(hash)))
        ),
        catchError((err) => {
    ...
    mainList$ = merge(this.onHash$, this.onPageChange$).pipe(
        switchMap(() =>
            this.dataProvider$.pipe(
                switchMap((provider) => provider(this.currentPage, this.hash))
            )
        ),
        retryWhen((errors) =>
            errors.pipe(
    ...
    filterList$ = this.filterProvider$.pipe(
        ...
        switchMap((filters) =>
            zip(
                ...
                this.onHash$.pipe(
                    switchMap((hash) =>
                        this.filterConfigProvider$.pipe(
                            switchMap((provider) => provider(hash))
                        )
                    )
                )
            )
        ...
        map(([filters, config]) => {
            ...
            filters.forEach((f) => {
                ...
                switch (f.type) {
                    case "flag":
                        const checkedIds = Object.keys(targetConf).map(
                            (id) => +id
                        );
                        ...
                        break;
                    case "select":
                        const selectedId = Object.keys(targetConf).map(
                            (id) => +id
                        )[0];
                        if (selectedId) {
    ...
    ngOnInit(): void {
        this.onHashError$
            .pipe(
                ...
                tap(() =>
                    this.router
                        .navigate([], { queryParams: { hash: null } })
                        .then()
                )
            )
            .subscribe();
